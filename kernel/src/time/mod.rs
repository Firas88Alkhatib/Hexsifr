...
use crate::time::calibration::{calibrate_lapic_timer, calibrate_tsc_frequency};
...
pub fn time_init() {
    let tsc_freq = calibrate_tsc_frequency().expect("Failed to calibrate TSC frequency");
    let boot_tsc = read_tsc_counter();
    TSC_FREQ.store(tsc_freq, Ordering::Relaxed);
    BOOT_TSC.store(boot_tsc, Ordering::Relaxed);

    let bios_date_time_nanos = cmos::read_rtc().to_nanos();
...
pub fn sleep_us(microseconds: u64) {
    ...
    let ticks = (tsc_freq() as u128 * microseconds as u128) / 1_000_000;
