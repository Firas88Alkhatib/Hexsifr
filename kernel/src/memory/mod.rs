...
use crate::memory::{frame_allocator::LLFreeFrameAllocator, heap_allocator::init_heap};

static PHYSICAL_MEMORY_OFFSET: AtomicU64 = AtomicU64::new(0);

pub(crate) fn memory_init(memory_regions: &mut MemoryRegions, num_cpus: usize) {
    info!("Initializing frame allocator");
    let mut frame_allocator = LLFreeFrameAllocator::new(memory_regions, num_cpus).expect("Cannot init frame allocator LLFree");

    init_heap(&mut frame_allocator).expect("Failed to initialize heap allocator");
}
