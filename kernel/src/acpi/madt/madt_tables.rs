...
pub trait LapicFlags {
    const ENABLED: u32 = 1;
    const ONLINE_CAPABLE: u32 = 2;
    fn flags(&self) -> u32;
    fn is_enabled(&self) -> bool {
        self.flags() & Self::ENABLED != 0
    }
    fn is_online_capable(&self) -> bool {
...
impl LapicFlags for MadtProcessorLocalApic {
    fn flags(&self) -> u32 {
        self.flags
    }
}
...
impl LapicFlags for MadtProcessorLocalX2Apic {
    fn flags(&self) -> u32 {
        self.flags
    }
}
...
impl Madt {
    ...
    pub fn lapic_address(&self, madt_address: u64) -> u64 {
        let mut lapic_address = self.local_apic_address as u64;

        for (header, entry_address) in self.entries(madt_address) {
            match header.entry_type {
                madt_entry_type::LOCAL_APIC_ADDRESS_OVERRIDE => {
                    ...
                    lapic_address = lapic_addr_override.local_apic_address;
                }
                _ => {}
            }
        }
